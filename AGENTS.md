...
- `version` is `2.6.13` and must stay in sync with `Cargo.toml` + `Cargo.lock` (`package.json` too).
...
### 4.3 Android CI Optimization (`.github/workflows/build.yml`) — ✅ DONE (2026-08-27, v2.5.18)

- APKs are built for **both `aarch64` and `x86_64` via `--split-per-abi`** (`cargo tauri android build --apk --target aarch64 x86_64 --split-per-abi`, `auralis-v2.5.18-android-arm64.apk` + `-x86_64.apk`); `x86_64` powers emulator E2E `pixel_6 api33 google_apis`.
...
- E2E note: `test-android-e2e` now verifies **player-working** via `e2e_player_test.js` (was `e2e_download_test.js`, renamed v2.5.13) — seeds `AudioTrack` from `/sdcard/{Music,Download}` (`SIDELoad` → `scan_library_paths` → `get_tracks` → `play`/`set_queue`/`Next`/`Prev` + DOM `data-role` tap), no YouTube network dependency. Desktop `desktop_download_player_e2e.js` seeds 2 tracks + verifies `set_queue` queue length `2` + modal hydration.

**Verify**: `gh release view v2.5.18` shows both `arm64` + `x86_64` APKs + desktop artifacts.
