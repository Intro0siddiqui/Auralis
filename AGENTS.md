...
- `version` is `2.5.8` and must stay in sync with `Cargo.toml` + `Cargo.lock` (`package.json` too).
...
- APKs are built for **both `aarch64` and `x86_64` via `--split-per-abi`** (`cargo tauri android build --apk --split-per-abi`, `auralis-v2.5.8-android-arm64.apk` + `-x86_64.apk`); `x86_64` powers emulator E2E `pixel_6 api33 google_apis`.
