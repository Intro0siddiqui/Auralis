...
SANDBOX_DOWNLOAD_DIR="/data/data/${PACKAGE_NAME}/downloads"
PORT="9222"
...
DOWNLOAD_FILES=$(adb shell "ls -l /data/data/${PACKAGE_NAME}/downloads/*.* /data/user/0/${PACKAGE_NAME}/downloads/*.* /data/data/${PACKAGE_NAME}/files/downloads/*.* 2>/dev/null" | grep -E '\.(m4a|mp3|webm|opus|ogg)$' | tr -d '\r' || true)
if [ -z "$DOWNLOAD_FILES" ] || echo "$DOWNLOAD_FILES" | grep -q "No such file"; then
    # Try run-as com.auralis.v2 fallback
    DOWNLOAD_FILES=$(adb shell "run-as ${PACKAGE_NAME} ls -l downloads/ 2>/dev/null" | grep -E '\.(m4a|mp3|webm|opus|ogg)$' | tr -d '\r' || true)
fi
...
if [ -z "$DOWNLOAD_FILES" ] || echo "$DOWNLOAD_FILES" | grep -q "No such file"; then
    echo "Error: No audio file found in sandboxed storage /data/data/${PACKAGE_NAME}/downloads/"
    exit 1
fi
...
if [ -z "$FILE_SIZE" ] || ! [[ "$FILE_SIZE" =~ ^[0-9]+$ ]]; then
    FILE_SIZE=$(adb shell "stat -c %s /data/data/${PACKAGE_NAME}/downloads/*.* /data/user/0/${PACKAGE_NAME}/downloads/*.* 2>/dev/null" | head -n 1 | tr -d '\r' || true)
fi
